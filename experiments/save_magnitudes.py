...
def main():
    ...
    for layer in layers:
        ...
        # PCA
        activation_matrix[layer] = np.mean(activation_matrix[layer],axis=0)
        print(layer,activation_matrix[layer].shape)
    ...
    pickle.dump(activation_matrix,open(os.path.join(args.data_path,args.experiment_name,args.model_name,'magnitude.pkl'),'wb'))
